...
| K-C1…C5 | `PlatformProfile` drives WAKE/DETECTED honestly; never downloads | — | **BUILT (master-package pass, 2026-09-25 evening):** watching signals from the Phase 0 dumps (`video_player_progress` etc.) drive `CORE_DETECT detected=true` → WAKE → DETECTED, honest IDLE off-video, 1.2 s flip debounce; detection still never downloads. Device verdict pending |
| K-D1…D5 | Real progress, duplicate check before tap, PAUSED behind D3 | — | **BUILT (master-package pass):** `downicore/CoreJobBinding` observes the service's own `dropLive` snapshot read-only (ONE JOB, ONE SOURCE OF TRUTH) — running → perimeter = the real percent, done → COMPLETING→COMPLETE, failed → FAILED, canceled → IDLE; 7 JVM tests. Duplicate check BEFORE job creation: a running same-URL job is adopted (`CHAIN_DELIVER_ADOPT`), a just-completed one is refused (`suppressed=already_saved`). PAUSED stays **D3-gated**: real pause/resume needs engine surgery (`.part` continuation — tiktok direct opens `'wb'`, yt-dlp runs `nopart:True`) which The Law reserves for the full device matrix; the visual states exist, nothing fake drives them |
